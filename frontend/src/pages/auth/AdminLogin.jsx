...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Welcome to the Admin Console, ${user.name}!`);
    ...
    <AuthLayout title="Admin Console">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5 text-left">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Admin Email</label>
        ...
        >
          {loading ? <Spinner size="sm" color="white" /> : 'Admin Log In'}
        </button>

        <div className="mt-4 text-center text-sm font-semibold text-slate-600 dark:text-slate-400">
          Need an Admin account?{' '}
          <Link to="/admin/register" className="text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 transition-colors">
            Register admin
          </Link>
        </div>
      </form>
